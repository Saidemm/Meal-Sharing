  ...
  try{
      ...
      if(availableReservations = 'true') {
        const mealsHasAvailableReservations = await knex("meal")
          ...
          .sum({ total_reserved: 'reservation.number_of_guests' })
...
router.get("/:id", async (request, response) => {
  try {
    ...
    const mealById = await knex("meal").where({id: mealId });
...
router.put("/:id", async (request, response) => {
  try {
    ...
    const updatedMeal = await knex("meal").where({id: mealId }).update(request.body);
